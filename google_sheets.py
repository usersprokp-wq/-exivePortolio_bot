...
class GoogleSheetsManager:
    ...
    def export_bonds_portfolio(self, portfolio_data):
        ...
        try:
            ...
            headers = ['Номер ОВДП', 'Термін до', 'Кількість', 'Середня ціна', 'Сума', 'Платформа']
    ...
    def import_bonds_portfolio_from_sheets(self):
        """Імпортує повний портфель ОВДП з аркуша ОВДП-Портфель (включно із залишками)"""
        try:
            worksheet = self.get_or_create_worksheet("ОВДП-Портфель")
            all_rows = worksheet.get_all_values()
            ...
            for row in all_rows[1:]:
                ...
                try:
                    portfolio.append({
                        ...
                        'total_quantity': int(row[2]) if len(row) > 2 and row[2] else 0,
                    ...
                    logger.warning(f"Error parsing portfolio row: {e}, row={row}")
            
            logger.info(f"Imported {len(portfolio)} records from 'ОВДП-Портфель'")
            return portfolio
        except Exception as e:
            logger.error(f"Error importing bond portfolio: {e}")
            return []
    
    def import_stocks_portfolio_from_sheets(self):
        """Імпортує повний портфель Акцій з аркуша Акції-Портфель (включно із залишками)"""
        try:
            worksheet = self.get_or_create_worksheet("Акції-Портфель")
            all_rows = worksheet.get_all_values()
            ...
            for row in all_rows[1:]:
                ...
                try:
                    portfolio.append({
                        ...
                        'total_quantity': int(row[1]) if len(row) > 1 and row[1] else 0,
            ...
            logger.info(f"Imported {len(portfolio)} records from 'Акції-Портфель'")
            return portfolio
        except Exception as e:
            logger.error(f"Error importing stocks portfolio: {e}")
            return []